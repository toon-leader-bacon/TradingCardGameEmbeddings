...
class _StubDataConstructor:
    ...
    of the row's actual content - isolates MultiGroupRegressionDojo's own
    wiring (splits/batching/loss) from any particular DataConstructor."""

    def __init__(self, group: MultiGroupInput, label: float) -> None:
        self._group = group
        self._label = label

    def build(self, chunk: pd.DataFrame, lookup: CardLookup) -> List[TrainingDatum]:
        return [(self._group, self._label) for _ in range(len(chunk))]
...
class TestMultiGroupRegressionDojoSplits:
    def test_training_data_yields_batches_covering_all_rows(
        self, tmp_path: Path
    ) -> None:
        source = tmp_path / "source.parquet"
        _write_source(source, num_rows=100)
        group = [[_card("A")], []]  # empty second group - must not break splitting
        dojo = MultiGroupRegressionDojo(
            path_to_training_data=source,
            data_constructor=_StubDataConstructor(group, 1.0),
    ...
    def test_test_and_validation_data_are_disjoint_from_training(
        ...
        dojo = MultiGroupRegressionDojo(
            path_to_training_data=source,
            data_constructor=_StubDataConstructor(group, -2.0),
            card_lookup=CardBinder(),
            holdout=HoldoutSpec.no_holdout(),
            card_embedding_size=4,
            config=DojoConfig(
...
class TestMultiGroupRegressionDojoPooler:
    def test_pooler_is_forwarded_to_decoder_head(self, tmp_path: Path) -> None:
        ...
        dojo = MultiGroupRegressionDojo(
            path_to_training_data=source,
            data_constructor=_StubDataConstructor(group, 0.0),
            card_lookup=CardBinder(),
            holdout=HoldoutSpec.no_holdout(),
            card_embedding_size=4,
            pooler=stub_pooler,
...
class TestMultiGroupRegressionDojoComputeLoss:
    ...
    def test_raises_on_mismatched_lengths(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(ValueError):
            dojo.compute_loss(
                [[[torch.randn(4)], []]], Batch([group] * len([0.0, 1.0]), [0.0, 1.0])
            )
