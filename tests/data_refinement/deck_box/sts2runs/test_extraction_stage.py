...
def _run_row(run_id: int, players: list[dict]) -> dict:
    return {"_serverId": run_id, "win": True, "players": players}
...
class TestExtract:
    def test_all_resolvable_cards_creates_deck(self, tmp_path: Path) -> None:
        ...
        _write_runs_gz(raw_path, [_run_row(1, [_player(["STRIKE", "DEFEND"])])])
    ...
    def test_unresolvable_card_falls_back_to_unknown_sentinel(
        ...
        box = DeckBox()
        raw_path = tmp_path / "runs.json.gz"
        _write_runs_gz(raw_path, [_run_row(1, [_player(["STRIKE", "MISSING_CARD"])])])
        stage = Sts2RunsDeckExtractionStage()
    ...
    def test_reordered_but_same_content_card_list_is_not_reported_as_changed(
        ...
        binder = _spire_codex_card_binder(["STRIKE", "DEFEND"])
        box = DeckBox()
        first_path = tmp_path / "runs.json.gz"
        _write_runs_gz(first_path, [_run_row(1, [_player(["STRIKE", "DEFEND"])])])
        stage = Sts2RunsDeckExtractionStage()
        ...
        reordered_path = tmp_path / "runs_reordered.json.gz"
        _write_runs_gz(reordered_path, [_run_row(1, [_player(["DEFEND", "STRIKE"])])])
    ...
    def test_raises_runtime_error_when_unknown_sentinel_not_seeded(
        ...
        box = DeckBox()
        raw_path = tmp_path / "runs.json.gz"
        _write_runs_gz(raw_path, [_run_row(1, [_player(["MISSING_CARD"])])])
        stage = Sts2RunsDeckExtractionStage()

        with pytest.raises(RuntimeError):
            stage.extract(raw_path, box, binder)

    def test_multiple_players_in_one_run_produce_distinct_decks(
        self, tmp_path: Path
    ) -> None:
        # sts2runs' schema allows more than one entry in "players" even
        # though every row observed in the live dump has exactly one —
        # extract() must not assume players[0] is the only entry.
        binder = _spire_codex_card_binder(["STRIKE", "DEFEND"])
        box = DeckBox()
        raw_path = tmp_path / "runs.json.gz"
        _write_runs_gz(
            raw_path,
            [_run_row(1, [_player(["STRIKE"]), _player(["DEFEND"])])],
        )
        stage = Sts2RunsDeckExtractionStage()
        ...
        decks = [box.get_by_uuid(uuid) for uuid in changed_uuids]
        strike_uuid = binder.get_by_alias(
            GameId.SLAY_THE_SPIRE_2, DataSource.SPIRE_CODEX, "STRIKE"
        ).nocab_uuid
        defend_uuid = binder.get_by_alias(
            GameId.SLAY_THE_SPIRE_2, DataSource.SPIRE_CODEX, "DEFEND"
        ).nocab_uuid
