...
from src.data_refinement.seventeenlands.v2.game_data_metrics.average_copies_when_included_metric import (  # noqa: E501 -- module path is one unbreakable dotted identifier
...
class TestAccumulateAndFinalize:
    def test_averages_copies_over_inclusions_only(self, tmp_path: Path) -> None:
        binder = CardBinder()
        bolt = binder.add(_card("Bolt")).stored_card
        csv_path = tmp_path / "game_data.csv"
        _write_csv(csv_path, deck_counts=[2, 1, 0, 3])  # 0 excluded from average
        output_path = tmp_path / "out.jsonl"
        metric = AverageCopiesWhenIncludedMetric(
        ...
        assert rows[0]["value"] == 2.0  # (2+1+3)/3

class TestDefaultOutputPath:
    def test_matches_default_output_dir_and_name(self) -> None:
        expected = (
            AverageCopiesWhenIncludedMetric.DEFAULT_OUTPUT_DIR
            / "MSH.PremierDraft.average_copies_when_included.jsonl"
        )
        assert (
            AverageCopiesWhenIncludedMetric.default_output_path("MSH", "PremierDraft") == expected
        )
