...
from src.data_refinement.seventeenlands.v2.replay_data_metrics.candidate_hand_mulligan_rate_metric import (  # noqa: E501 -- module path is one unbreakable dotted identifier
...
def _write_csv(path: Path, rows: list[tuple[int, str]]) -> None:
    lines = [_HEADER]
    for num_mulligans, candidate_hand_1 in rows:
        lines.append(f"True,{num_mulligans},,{candidate_hand_1}\n")
    path.write_text("".join(lines))

class TestAccumulateAndFinalize:
    def test_computes_mulligan_rate(self, tmp_path: Path) -> None:
        binder = CardBinder()
        bolt = _register_arena_card(binder, "Bolt", "1")
        csv_path = tmp_path / "replay.csv"
        _write_csv(csv_path, rows=[(1, "1"), (0, "1"), (1, "1")])
        output_path = tmp_path / "out.jsonl"
        metric = CandidateHandMulliganRateMetric(
    ...
    def test_default_output_path(self) -> None:
        expected = (
            CandidateHandMulliganRateMetric.DEFAULT_OUTPUT_DIR
            / "MSH.PremierDraft.candidate_hand_mulligan_rate.jsonl"
        )
        assert (
            CandidateHandMulliganRateMetric.default_output_path("MSH", "PremierDraft") == expected
        )
