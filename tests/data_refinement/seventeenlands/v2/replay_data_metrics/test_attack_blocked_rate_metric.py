...
from src.data_refinement.seventeenlands.v2.replay_data_metrics.attack_blocked_rate_metric import (
    AttackBlockedRateMetric,
)
...
def _write_csv(path: Path, rows: list[dict[str, str]]) -> None:
    columns = [
        ...
        "oppo_turn_2_creatures_attacked",
    ]
    lines = [",".join(columns) + "\n"]
    for row in rows:
...
class TestAccumulateAndFinalize:
    def test_computes_blocked_rate_per_attack_instance(self, tmp_path: Path) -> None:
        binder = CardBinder()
            ...
            rows=[
                {
                    ...
                    "user_turn_1_creatures_blocked": "1",
                    "oppo_turn_2_creatures_attacked": "2",
                }
            ],
        )
        output_path = tmp_path / "out.jsonl"
        metric = AttackBlockedRateMetric(binder, GameId.MTG, "MSH", "PremierDraft", output_path)
    ...
    def test_default_output_path(self) -> None:
        expected = (
            AttackBlockedRateMetric.DEFAULT_OUTPUT_DIR
            / "MSH.PremierDraft.attack_blocked_rate.jsonl"
        )
        assert AttackBlockedRateMetric.default_output_path("MSH", "PremierDraft") == expected
