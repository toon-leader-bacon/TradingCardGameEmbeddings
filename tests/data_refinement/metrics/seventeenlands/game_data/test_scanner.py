"""Skeleton-stage test stubs for scanner.py's scan_game_csv() - the
same isolation contract as
tests/data_refinement/metrics/seventeenlands/draft_data/'s (implicit,
via draft_data/scanner.py's scan_draft_csv()) tests. Bodies are filled
in by design-recipe-implement.
"""
...
class _RecordingMetric:
    ...
    sees, for asserting scan_game_csv()'s driving contract."""
...
def test_one_metrics_accumulate_failure_is_isolated_from_the_others(
    ...
    scan_game_csv(csv_path, [raising_metric, healthy_metric])
...
def test_every_metric_is_finalized_after_the_full_scan(tmp_path: Path) -> None:
    ...
    scan_game_csv(csv_path, [first_metric, second_metric])
