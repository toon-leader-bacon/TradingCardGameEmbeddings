...
class FileManagerCSV:
    ...
    def __init__(
        ...
        if not self.path_to_training_data.suffix == ".csv":
            raise ValueError(
                f"Training data file is not a CSV: {self.path_to_training_data}"
            )
        ...
        self.header_row: Optional[List[str]]
        if self.has_header_row:
            # Read the header row from the file
            self.header_row = pd.read_csv(
                self.path_to_training_data, nrows=1
            ).columns.tolist()
        else:
    ...
    def make_splits(
        ...
        """
        splits: TTVSplits[pd.Series] = TTVSplits.from_unnormalized(split_ratios)
        ...
        for chunk in reader:
            ...
            for i, (start_index, end_index) in enumerate(
                splits.get_split_indices(len(chunk))
            ):
    ...
    def shuffle_split(self, split: Split, batch_size: int = 32):
    ...
    def _prepare_split_output_files(
        ...
        output_paths = [
            self.output_directory
            / f"{self.output_file_prefix}_{get_split_file_postfix(i)}.csv"
            for i in range(num_splits)
        ]
        header_frame = pd.DataFrame(
            columns=self.header_row if self.has_header_row else []
        )
