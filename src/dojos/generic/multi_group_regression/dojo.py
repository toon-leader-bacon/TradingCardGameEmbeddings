"""Generic dojo for the (two card groups in, scalar regression out)
task shape. Sibling to multi_group_option_selection/dojo.py - same
MultiGroupInput = [group_0, group_1] shape and the same group-ordering
requirement (see AttackerBlockerCombatOutcomeDataConstructor,
src/dojos/generic/data_constructors.py, for why the never-empty group
must stay index 0) - but pools both groups into one aggregate scalar
via MultiGroupRegressionDecoderHead instead of scoring individual
members of one group; see that decoder head's own module docstring for
the structural distinction.

Today's only consumer is AttackerBlockerCombatOutcomeMetric, via the
thin wrapper at src/dojos/seventeenlands/replay_data/
attacker_blocker_combat_outcome_dojo.py.
"""
...
class MultiGroupRegressionDojo:
    ...
    decoder head or loss themselves - regression always uses MseLoss,
    same as MultiCardRegressionDojo. pooler is this cell's one swappable
    seam, forwarded straight through to the decoder head it builds."""
    ...
    def __init__(
        ...
        Inputs:
            path_to_training_data: a metric's own output parquet file
                (e.g. AttackerBlockerCombatOutcomeMetric.DEFAULT_OUTPUT_PATH).
            data_constructor: converts raw rows from that file into
                (MultiGroupInput, float) TrainingDatum pairs (e.g.
            ...
            pooler: the EmbeddingPooler strategy forwarded to this
                dojo's decoder head (see pooling.py) - one shared
                instance pools BOTH card groups (see
                MultiGroupRegressionDecoderHead's own docstring). None
        ...
        self.decoder_head = MultiGroupRegressionDecoderHead(
    ...
    def compute_loss(
        ...
        Inputs:
            embeddings: one example's [group_0_embeddings,
                group_1_embeddings] pair per training example, same
                order as labels. len(embeddings) must equal len(labels).
            labels: the ground-truth float label for each example, as
    ...
    def _data_iterator(
        ...
        cell's _data_iterator this session (e.g.
        MultiGroupOptionSelectionDojo's) - copied rather than shared,
        per this project's existing precedent (see that class's own
        _data_iterator docstring for the near-identical-logic tradeoff
        this repeats).
