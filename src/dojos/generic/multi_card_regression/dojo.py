"""Generic dojo for the (whole deck in, scalar regression out) task
shape.

Same shape as SingleCardRegressionDojo (src/dojos/generic/
single_card_regression/dojo.py) but for MultiCardInput instead of
SingleCardInput - today that's DeckLabelMetric's int64-labeled concrete
metrics (src/dojos/sts_gg/deck_label_dojos.py's thin wrappers, see
plans/dojo_v2.md). Takes an injected DataConstructor instead of owning
one, so this one class serves every metric family whose training input
is a whole deck and whose label is a plain int/float.
"""
...
class MultiCardRegressionDojo:
    ...
    implementation detail (regression always uses MseLoss, same as
    SingleCardRegressionDojo). The one addition over the single-card
    sibling is an optional pooler, forwarded straight through to the
    decoder head it builds - the pooling strategy is the decoder head's
    own concern (see decoder_head.py), this dojo just passes it along."""
    ...
    def __init__(
        ...
        Inputs:
            path_to_training_data: a metric's own output parquet file
                (e.g. one DeckLabelMetric subclass's DEFAULT_OUTPUT_PATH).
            data_constructor: converts raw rows from that file into
                (MultiCardInput, int) TrainingDatum pairs - the
                metric-family-specific collaborator (e.g.
                DeckLabelDataConstructor) a thin per-metric wrapper
        ...
        self.decoder_head = MultiCardRegressionDecoderHead(
    ...
    def compute_loss(
        ...
        Inputs:
            embeddings: one deck's card embeddings per training
                ...
                equal len(labels).
            labels: the ground-truth int/float label for each deck, as
    ...
    def _data_iterator(
        ...
        SingleCardRegressionDojo._data_iterator() - see that method for
