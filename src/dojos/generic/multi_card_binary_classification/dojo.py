"""Generic dojo for the (whole deck in, single logit binary
classification out) task shape.

Structurally identical to MultiCardRegressionDojo (src/dojos/generic/
multi_card_regression/dojo.py) - same constructor/method shape, same
optional pooler forwarded to the decoder head - the only differences
are which loss (BceLoss, not MseLoss) and which decoder head
(MultiCardBinaryClassificationDecoderHead, not
MultiCardRegressionDecoderHead) this dojo builds internally. Today
that's DeckLabelMetric's WinMetric (src/dojos/sts_gg/
deck_label_dojos.py's WinDojo, see plans/dojo_v2.md) - this supersedes
the plan's earlier decision to route WinMetric through
MultiCardFixedClassificationDojo's 2-class cross-entropy instead.
"""
...
class MultiCardBinaryClassificationDojo:
    ...
    implementation detail (binary classification always uses BceLoss
    against a raw logit, same wiring shape MultiCardRegressionDojo uses
    for MseLoss). The one addition over a bare fixed-classification cell
    is an optional pooler, forwarded straight through to the decoder
    head it builds - the pooling strategy is the decoder head's own
    concern (see decoder_head.py), this dojo just passes it along."""
    ...
    def __init__(
        ...
        Inputs:
            path_to_training_data: a metric's own output parquet file
                (e.g. WinMetric's DEFAULT_OUTPUT_PATH).
            data_constructor: converts raw rows from that file into
                (MultiCardInput, float) TrainingDatum pairs, label in
                {0.0, 1.0} - the metric-family-specific collaborator
                (e.g. DeckLabelDataConstructor) a thin per-metric
                wrapper configures and injects.
        ...
        self.loss_calculator = BceLoss()
        self.decoder_head = MultiCardBinaryClassificationDecoderHead(
    ...
    def compute_loss(
        ...
        """Compute the binary classification loss between predicted
        embeddings and true labels.

        Inputs:
            embeddings: one deck's card embeddings per training
                example, same order as labels. len(embeddings) must
                equal len(labels).
            labels: the ground-truth 0.0/1.0 label for each deck, as
    ...
    def _data_iterator(
        ...
        methods above. Identical shape to
        MultiCardRegressionDojo._data_iterator() - see that method for
        the reasoning behind calling the mod pipeline unconditionally
        with is_training rather than gating it behind an if.
