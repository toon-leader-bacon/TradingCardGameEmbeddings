"""Thin MetricRegressionDojo wrapper for replay_data_metrics' own
OpeningHandWinRateMetric.

Lives under replay_data_dojo/ — game_data_metrics has a distinct
...
game_data_dojo/opening_hand_win_rate_dojo.py. The per-source directory
(mirroring data_refinement/seventeenlands/'s own game_data_metrics/ vs.
replay_data_metrics/ split) is what disambiguates the two — same
precedent as the two source Metric classes themselves, which already
share this exact name across modules.
...
class OpeningHandWinRateDojo(MetricRegressionDojo):
    """Regresses a card's embedding against replay_data_metrics'
    ...
    fixes metric_name to OpeningHandWinRateMetric.name (this replay_
    data_metrics variant, never a re-declared string literal — this
    pairing can't drift) and resolves a default metrics_path/loss so
    the common case needs no extra wiring.
    """
    ...
    def __init__(
        ...
        replay_data_metrics opening_hand_win_rate values.
        ...
        Inputs:
            ...
            metrics_path: which parquet file to read
                ...
                ReplayMetricScanner.default_output_path(expansion,
                format_code) (this project's conventional replay_data_
