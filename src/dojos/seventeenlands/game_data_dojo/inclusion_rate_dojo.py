"""Thin MetricRegressionDojo wrapper for InclusionRateMetric
(game_data_metrics).
...
class InclusionRateDojo(MetricRegressionDojo):
    """Regresses a card's embedding against its inclusion_rate.
    ...
    fixes metric_name to InclusionRateMetric.name (never a re-declared
    string literal — this pairing can't drift) and resolves a default
    metrics_path/loss so the common case needs no extra wiring.
    """
    ...
    def __init__(
        ...
        Inputs:
            ...
            metrics_path: which parquet file to read inclusion_rate
                rows from. Defaults to MetricScanner.default_output_path(
                expansion, format_code) (this project's conventional
        ...
        super().__init__(
            ...
            metric_name=InclusionRateMetric.name,
