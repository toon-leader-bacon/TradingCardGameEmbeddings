"""Drives any number of v2 Metrics through one chunked pass over a raw CSV.

See plans/metrics_v2.md for the full design this codifies. Unlike
src/data_refinement/seventeenlands/game_data_metrics/metric_scanner.py
(the v1 scanner, still in active use, not modified by this file), this
class has no knowledge of column resolution, checkpointing, or output
writing — every one of those is now a Metric's own concern (metric.py).
This class's entire job is streaming chunks and driving every active
metric's accumulate()/finalize() over them, so N active metrics still
cost one pass over the file, not N passes (the one property carried
forward unchanged from v1).

Named CsvScanner, not just Scanner, because it's specific to CSV raw
data — a future JSONL/Kafka/etc. raw shape would get its own scanner
class, not a generalization of this one (see plans/metrics_v2.md's
directory-layout note on why csv_scanner.py stays flat, not nested
under a scanners/ subdirectory, until a second scanner type actually
exists).
"""
...
class CsvScanner:
    ...
    def __init__(self, raw_csv_path: Path, chunk_size: int, metrics: list[Metric]) -> None:
        """
        Inputs:
