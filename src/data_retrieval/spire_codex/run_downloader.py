...
- `limit` bounds a page to at most that many runs (server max 50000).
  ...
  since fetch() always pages explicitly so a slow full-corpus pull can
...
per page: fetch() also needs to know the cursor a resumed walk should
...
_WINDOW_METADATA_FILENAME) the first time fetch() is called against a
given raw_data_dir, and every later fetch() call against that same
...
above), so fetch() separately calls GET /api/runs/stats once, purely
...
best-effort and purely cosmetic — if it fails, fetch() falls back to
...
class SpireCodexRunDownloader:
    ...
    def __init__(
        ...
        *,
        ...
        Inputs:
            ...
            raw_data_dir: directory each page file (its cursor
                sidecar, and the walk's window metadata) is written
            ...
            stats_url: URL of the community-stats endpoint, queried
                only to approximate a total page count for fetch()'s
            ...
            rate_limiter: paces every outgoing request this class makes
                (each page request, plus the one-off stats call).
                Passed in rather than constructed internally
                (dependency injection — PATTERNS.md), same as
                HearthstoneJsonDownloader, so the same shared
                RateLimiter instance can be reused across sources and
                tests can supply a fast/no-op limiter. Required, not
                optional: manual testing hit a live 429 from this
                server despite no documented rate limit (see this
                module's docstring) — every external source in this
                container self-paces rather than assuming it's safe
                not to.
    ...
    def fetch(self, *, start: str | None = None, end: str | None = None) -> list[Path]:
        ...
        Output: path to every page file for this corpus/window, in
            page order — both pages already on disk from a prior call
            and pages newly fetched this call.
        Side effects: creates raw_data_dir if missing; one best-effort,
            ...
            first time fetch() is ever called against this
        ...
        Exceptions: raises ValueError if raw_data_dir already holds
            ...
            doesn't match the window passed to this call. Raises on
            network failure (e.g. connection error, non-2xx response)
            or on failure to write a file. A paged walk is a dependent
            chain (page N+1 needs page N's cursor), so unlike
            HearthstoneJsonDownloader this is not best-effort: one page
            failing stops the whole call, and a later fetch() call
        ...
        page_paths = [page.path for page in downloaded_pages]
        ...
        with tqdm(
            ...
            while True:
                page = self._fetch_page(page_index, cursor, start=start, end=end)
    ...
    def _scan_downloaded_pages(self) -> list[_FetchedPage]:
        ...
        Private helper — single consumer is fetch(). A page counts as
        ...
        not present, so fetch() re-fetches it. Pages are a contiguous
    ...
    def _resume_point(self, downloaded_pages: list[_FetchedPage]) -> _ResumePoint:
        ...
        Private helper — single consumer is fetch(). Pure: takes
        _scan_downloaded_pages()'s own output rather than re-reading
        raw_data_dir itself, so this can't drift from what fetch()
    ...
    def _record_window(self, start: str | None, end: str | None) -> None:
        ...
        Private helper — single consumer is fetch(), called
    ...
    def _estimate_total_pages(self) -> int | None:
        """Best-effort, approximate total page count for fetch()'s
        progress bar.

        Private helper — single consumer is fetch(). Purely cosmetic:
        this is never allowed to fail fetch() itself, since the actual
    ...
    def _fetch_page(
        ...
        Private helper — single consumer is fetch(). Not built on
